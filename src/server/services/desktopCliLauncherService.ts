...
const DESKTOP_CLI_NAME = 'sdx'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> SDX PATH >>>'
const PATH_BLOCK_END = '# <<< SDX PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "sdx launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo sdx launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
