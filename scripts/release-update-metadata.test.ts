...
describe('release update metadata merge', () => {
  test('merges namespaced macOS x64 and arm64 metadata into standard latest-mac.yml', () => {
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-arm64.zip
          sha512: arm64-checksum
          size: 222
      path: SDX-0.3.2-arm64.zip
      sha512: arm64-checksum
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-x64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-x64.zip
          sha512: x64-checksum
          size: 111
      path: SDX-0.3.2-x64.zip
      sha512: x64-checksum
    ...
    expect(merged.path).toBe('SDX-0.3.2-x64.zip')
  ...
  test('restores standard linux channel names for x64 and arm64 metadata', () => {
    ...
    writeYaml(join(inputDir, 'latest-linux-Linux-x64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-x64.AppImage
          sha512: linux-x64-checksum
          size: 111
      path: SDX-0.3.2-x64.AppImage
      sha512: linux-x64-checksum
    `)
    writeYaml(join(inputDir, 'latest-linux-Linux-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-arm64.AppImage
          sha512: linux-arm64-checksum
          size: 222
      path: SDX-0.3.2-arm64.AppImage
      sha512: linux-arm64-checksum
    `)
    ...
    expect(x64.path).toBe('SDX-0.3.2-x64.AppImage')
    expect(arm64.path).toBe('SDX-0.3.2-arm64.AppImage')
  })
  ...
  test('keeps Linux AppImage as primary update artifact when deb and rpm are also published', () => {
    ...
    writeYaml(join(inputDir, 'latest-linux-Linux-x64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-linux-amd64.deb
          sha512: linux-deb-checksum
          size: 222
        - url: SDX-0.3.2-linux-x86_64.AppImage
          sha512: linux-appimage-checksum
          size: 111
        - url: SDX-0.3.2-linux-x86_64.rpm
          sha512: linux-rpm-checksum
          size: 333
      path: SDX-0.3.2-linux-amd64.deb
      sha512: linux-deb-checksum
    `)
    ...
    expect(x64.files.map(file => file.url)).toEqual([
      'SDX-0.3.2-linux-x86_64.AppImage',
      'SDX-0.3.2-linux-amd64.deb',
      'SDX-0.3.2-linux-x86_64.rpm',
    ])
    expect(x64.path).toBe('SDX-0.3.2-linux-x86_64.AppImage')
  ...
  test('restores standard Windows channel metadata after matrix namespacing', () => {
    ...
    writeYaml(join(inputDir, 'latest-Windows-x64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-win-x64.exe
          sha512: win-checksum
          size: 333
      path: SDX-0.3.2-win-x64.exe
      sha512: win-checksum
    `)
    writeYaml(join(inputDir, 'latest-Windows-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: SDX-0.3.2-win-arm64.exe
          sha512: win-arm64-checksum
          size: 222
      path: SDX-0.3.2-win-arm64.exe
      sha512: win-arm64-checksum
    `)
    ...
    expect(windows.files.map(file => file.url)).toEqual([
      'SDX-0.3.2-win-x64.exe',
      'SDX-0.3.2-win-arm64.exe',
    ])
    expect(windows.path).toBe('SDX-0.3.2-win-x64.exe')
