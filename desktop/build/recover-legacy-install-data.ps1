    ...
    foreach ($installDir in $InstallDirs) {
      if (Test-PathMayBeDeleted -InstallDir $installDir -Candidate $active) {
        ...
        if ($ActiveConfigManaged -ne '1') {
          throw "Active CLAUDE_CONFIG_DIR is managed outside SDX and points inside an application install directory. Move or remove that environment variable before upgrading: $active"
        }
...
function Run-SelfTest {
  ...
  try {
    ...
    try {
      ...
      $unmanagedFailed = $_.Exception.Message.Contains('managed outside SDX')
    }
