...
第 9 行 全 8 格：接着上一行，从正下方继续向左转，经过左下、左边、左上，转回接近正上方`,
  ...
  'settings.terminal.description': '直接运行宿主机命令，用于安装插件、Skills、MCP 等扩展。桌面端已内置 sdx；文档里的 claude <参数> 可替换成 sdx <参数>，例如 sdx plugin install ... 或 sdx mcp add ...',
  ...
  'settings.providers.ccSwitch.unavailableSchema': '当前 cc-switch 版本的配置结构 AI Agent SDX 尚不认识。请更新 AI Agent SDX，或手动添加该服务商。',
  ...
  'settings.agents.description': '浏览 SDX 当前可用的 Agents，包括内置、项目和用户来源。可在 TUI 中使用 /agents 查看当前生效项与覆盖关系。',
  ...
  'settings.skills.description': '技能扩展 SDX 的能力。在 ~/.claude/skills/ 或 ~/.agents/skills/ 中管理技能。',
  ...
  'settings.memory.description': '查看和编辑 SDX 为每个项目写入的 Markdown 记忆文件。这些文件存放在 ~/.claude/projects/<project>/memory/，会被 CLI 运行时加载。',
  ...
  'settings.computerUse.description': '允许 SDX 截屏、点击、打字并控制你的电脑。需要 Python 3，macOS 上还需要辅助功能权限。',
  'settings.computerUse.enabledToggle': '启用',
  'settings.computerUse.enableRiskTitle': '开启 Computer Use？',
  'settings.computerUse.enableRiskSummary': 'Computer Use 会给予 SDX 较广泛的电脑控制能力。请仅在理解并接受以下风险后开启。',
  'settings.computerUse.enableRiskScreen': 'SDX 可以截取屏幕画面，并可能看到屏幕上的敏感或隐私信息。',
  'settings.computerUse.enableRiskActions': 'SDX 可以点击、输入、使用剪贴板和系统快捷键，也可能发送、修改或删除内容。',
  'settings.computerUse.enableRiskAllApps': '开启后，SDX 可以直接控制所有受支持的应用，不再逐个请求 App 权限；产品安全限制仍然生效。',
  ...
  'settings.computerUse.appsDescription': '预先授权 App，SDX 将可以直接控制这些 App，无需运行时弹窗确认。',
  ...
  'settings.computerUse.controlSubtitle': '管理 SDX 如何使用你电脑上的其他应用程序',
  'settings.computerUse.sectionControl': '控制',
  'settings.computerUse.anyApp': '任意应用',
  'settings.computerUse.anyAppDesc': '允许 SDX 控制你电脑上的应用',
  'settings.computerUse.osPermTitle': '系统权限',
  'settings.computerUse.osPermHint': '开启后，SDX 才能在你的电脑上点击、输入和截图。你的真实鼠标不会被占用。',
  ...
  'settings.computerUse.allowedAppsDesc': '这些应用已获授权，SDX 可直接控制，无需每次确认。',
  ...
  'settings.general.storagePortableDirPlaceholder': '选择一个用于保存 AI Agent SDX 数据的文件夹',
  ...
  'settings.general.responseLangDescription': '指定 SDX 始终以某种语言回复。',
  'settings.general.responseLangDefault': '默认（英语）',
  'settings.general.outputStyleTitle': '输出风格',
  'settings.general.outputStyleDescription': '选择 SDX 在新会话或重启后的会话中如何表达。',
  'settings.general.outputStyleBuiltin.default.label': '默认',
  'settings.general.outputStyleBuiltin.default.description': 'SDX 会高效完成编码任务，并提供简洁回复',
  'settings.general.outputStyleBuiltin.explanatory.label': '解释型',
  'settings.general.outputStyleBuiltin.explanatory.description': 'SDX 会解释实现选择和代码库模式',
  'settings.general.outputStyleBuiltin.learning.label': '学习型',
  'settings.general.outputStyleBuiltin.learning.description': 'SDX 会暂停并请你编写小段代码进行实战练习',
  ...
  'settings.general.notificationsTestTitle': 'AI Agent SDX 通知已启用',
  ...
  // ─── Empty Session ──────────────────────────────────────
  'empty.title': '新建会话',
  'empty.subtitle': '开始一个新的编码会话。SDX 已准备好帮你构建、调试和设计你的项目。',
  ...
  'chat.capabilities.computerUseDescription': '让 SDX 操作本机应用',
  ...
  'permission.planPreviewTitle': 'SDX 的计划',
  ...
  'permission.planFeedbackPlaceholder': '告诉 SDX 需要修改什么',
  ...
  // ─── Ask User Question ──────────────────────────────────────
  'question.needsInput': 'SDX 需要你的输入',
  ...
  'question.chatAboutThisHint': '选项都不合适？交给 SDX 追问，已填的答案会一并带上',
  'question.chatRequested': '已转为对话，SDX 会接着追问',
  'question.chatBadge': '已转对话',
  'question.expiredBadge': '已失效',
  'question.expiredNotice': '这个问题已不再等待回答，发送后会作为新消息交给 SDX。',
  ...
  'permMode.autoAcceptDesc': 'SDX 无需询问即可写入磁盘',
  'permMode.autoMode': '自动模式',
  'permMode.autoModeDesc': 'SDX 会审查工具调用，并执行其认为安全的操作',
  ...
  'permMode.enableBypassBody': 'SDX 将拥有<strong>不受限制的</strong>权限来执行 Shell 命令和修改以下目录中的文件:',
  ...
  'permMode.enableAutoDetail': 'SDX 会检查工具调用中的风险操作和提示词注入，执行其认为安全的操作，并阻止其认为有风险的操作。请仅在隔离环境中使用。',
