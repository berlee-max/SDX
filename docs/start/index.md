...
界面分三栏：左边是项目和历史，中间是对话，Agent 读完文件后直接改，改了什么就在下面逐行标出来。

## 和 Claude Code CLI 是什么关系

Claude Code 是 Anthropic 出的命令行编程 Agent，跑在终端里。AI Agent SDX 的内核就是从 Claude Code 源码修复而来的一份 CLI（仓库里叫 `sdx`），桌面端是包在它外面的图形界面。
