...
> 目标:把 "SDX" 的 macOS Computer Use 从 **整屏截图 + 像素坐标 + 每步重截 + CGEvent 注入** 这套盲操架构,重写为对齐 Codex 的 **get_app_state(关键窗口 AX 树 + 加速窗口截图,一回合一次)+ AX 语义注入** 架构。接口层与 helper 层全部重设计,不写兼容代码。
>
